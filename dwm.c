...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			x = selmon->ww - statusw - 2 * sp - getsystraywidth();
			click = ClkStatusText;
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		tw = statusw = m->ww - drawstatusbar(m, bh, stext) - stw - 2 * sp;
	}
...
monocle(Monitor *m)
{
	...
	for (c = nexttiled(m->clients); c; c = nexttiled(c->next))
		resize(c, m->wx + sp + 1, m->wy + vp + 1, m->ww - 2 * c->bw - 2 * sp - 2, m->wh - 2 * c->bw - 2 * vp - 2, 0);
}
...
updatesystray(void)
{
	...
	if (!systray) {
		...
		XChangeProperty(dpy, systray->win, netatom[NetSystemTrayOrientation], XA_CARDINAL, 32,
				PropModeReplace, (unsigned char *)&netatom[NetSystemTrayOrientationHorz], bh);
		XChangeWindowAttributes(dpy, systray->win, CWEventMask|CWOverrideRedirect|CWBackPixel, &wa);
		XMapRaised(dpy, systray->win);
		XSetSelectionOwner(dpy, netatom[NetSystemTray], systray->win, CurrentTime);
		/* bh edit -- Mina */
		if (XGetSelectionOwner(dpy, netatom[NetSystemTray]) == systray->win) {
			sendevent(root, xatom[Manager], StructureNotifyMask, CurrentTime, netatom[NetSystemTray], systray->win, 0, bh);
			XSync(dpy, False);
		}
		else {
